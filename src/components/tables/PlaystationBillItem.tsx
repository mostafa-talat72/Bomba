    ...
    <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 rounded-lg transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg border-2 gap-3 sm:gap-0
      ...
      <div className="flex-1 cursor-pointer w-full sm:w-auto" onClick={() => onPaymentClick(bill)}>
        <div className="flex items-center gap-2 mb-2">
          <span className="font-semibold text-lg sm:text-xl text-gray-900 dark:text-gray-100">#{bill.billNumber || bill.id || bill._id}</span>
          <span className={`px-2 sm:px-3 py-1 text-base font-bold rounded-full shadow-sm ${getStatusColor(bill.status)}`}>{getStatusText(bill.status)}</span>
        </div>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 text-base sm:text-lg text-gray-600 dark:text-gray-400">
          {bill.table?.number ? (
      ...
      <div className="flex flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-end">
        <div className="text-center bg-white dark:bg-gray-800 px-2 sm:px-3 py-2 rounded-lg shadow-sm">
          <div className={`text-lg sm:text-xl font-bold ${isUnpaid ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'}`}>{formatCurrency(bill.remaining || 0)}</div>
          <div className="text-base text-gray-500 dark:text-gray-400">{t('billing.remainingAmount')}</div>
        </div>
        {onChangeTableClick && (
          <button onClick={(e) => { e.stopPropagation(); onChangeTableClick(bill); }}
            className="px-2 sm:px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-base sm:text-lg rounded-lg transition-all flex items-center gap-1 shadow-md">
