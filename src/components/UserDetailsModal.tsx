...
import { GROUP_ICONS } from './PermissionPicker';
              ...
              {user.permissions.includes('all') ? (
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    PERMISSION_GROUPS.forEach(g => g.permissionIds.forEach(id => grouped.add(id)));
                    const ungrouped = user.permissions.filter(id => !grouped.has(id) && id !== 'all');
                    const groups = ungrouped.length > 0
                      ? [...PERMISSION_GROUPS, { id: 'other', titleKey: 'users.permissionGroups.other', icon: 'other', permissionIds: ungrouped }]
                      : PERMISSION_GROUPS;
                    return groups.map(g => {
                      const items = g.permissionIds.filter(id => user.permissions.includes(id));
                      if (items.length === 0) return null;
                      const Icon = GROUP_ICONS[g.icon];
                      return (
                        <div
                          key={g.id}
                          className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-2 border-green-200 dark:border-green-800 rounded-xl p-4"
                        >
                          <div className="flex items-center gap-2 mb-3">
                            {Icon && (
                              <span className="p-1.5 rounded-lg bg-white dark:bg-gray-700 border border-green-200 dark:border-green-800 shadow-sm">
                            ...
                            <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300">
                              {items.length}
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            {items.map(id => {
                              const permInfo = permissions.find(p => p.id === id);
                              return (
                                <span
                                  key={id}
                                  title={permInfo?.description || id}
                                  className="px-2.5 py-1 bg-white dark:bg-gray-700 border border-green-200 dark:border-green-700 rounded-full text-xs font-bold text-green-800 dark:text-green-200 shadow-sm"
                                >
                                  {permInfo ? permInfo.name : id}
                                </span>
                              );
                            })}
                          </div>
