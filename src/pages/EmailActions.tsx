...
const EmailActions: React.FC = () => {
  ...
  const handleResendVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSuccessMessage(data.message || t('auth.verificationSentSuccess'));
        setEmail(''); // Clear email field
      } else {
        setErrors({ email: data.message || t('auth.resendVerificationError') });
      }
    } catch (error) {
      setErrors({ email: t('auth.emailActionError') });
    } finally {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSuccessMessage(data.message || t('auth.resetLinkSentSuccess'));
        setEmail(''); // Clear email field
      } else {
        setErrors({ email: data.message || t('auth.resetLinkError') });
      }
    } catch (error) {
      setErrors({ email: t('auth.emailActionError') });
    } finally {
  ...
  const getPageContent = () => {
    if (action === 'resend') {
      return {
        ...
        subtitle: t('auth.resendVerificationSubtitle'),
        icon: (
        ...
        buttonText: isSubmitting ? t('auth.sending') : t('auth.sendVerificationLink'),
      ...
      return {
        ...
        buttonText: isSubmitting ? t('auth.sending') : t('auth.sendResetLink'),
