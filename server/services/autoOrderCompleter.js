...
// الطلبات اللي ما اتمش تجهيزها أو توصيلها خلال 12 ساعة من إنشائها
// بتتجهز وتتوصّل أوتوماتيكيًا حتى لو شاشة المطبخ مقفولة.
const AUTO_COMPLETE_MS = 12 * 60 * 60 * 1000;
...
export const runAutoOrderCompleteOnce = async (io) => {
    ...
    for (const order of orders) {
        ...
        // updatedBy is ObjectId-typed: only carry it over when present.
        // Writing the "system" string here would corrupt the doc (and break
        // future Mongoose reads with a CastError).
        if (order.updatedBy) updateFields.updatedBy = order.updatedBy;
        ...
        // Check if item counts actually changed — but ALWAYS flip the status:
        // an order whose counts are already full yet stuck in pending would
        ...
        completed++;
        Logger.info(
            `[AutoComplete] Order #${result.orderNumber} auto-prepared & delivered (${result.items.length} items, older than 12h)`
        );
    }
...
export const startAutoOrderCompleter = (io) => {
    ...
    Logger.info("[AutoComplete] Scheduled job started (every 10 minutes, threshold 12h)");
};
