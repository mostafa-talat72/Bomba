...
const generateDailyReportForOrganization = async (organization) => {
    try {
        ...
        // استدعاء الدالة المشتركة من organizationController
        const { generateAndSendDailyReport } = await import('../controllers/organizationController.js');
        
        // Get organization timezone for locale
        const orgTimezone = organization.timezone || 'Africa/Cairo';
        const userLocale = orgTimezone.includes('Cairo') ? 'ar-EG' : 'en-US';
        
        Logger.info(`📧 Calling generateAndSendDailyReport for ${organization.name}...`);
        
        const result = await generateAndSendDailyReport(organization._id, userLocale);
        
        // Check if the report was skipped
        if (result.skipped) {
            Logger.info(`⏭️ Daily report skipped for ${organization.name}: ${result.reason}`);
            return;
        }
        
        Logger.info(`✅ Daily report sent successfully for ${organization.name}`, {
            organizationId: organization._id,
