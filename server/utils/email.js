...
export const sendEmail = async (options) => {
  ...
  try {
    const mailOptions = {
      from: `"Bastira System" <${process.env.EMAIL_USER}>`,
      to: options.to,
...
export const emailTemplates = {
  // Low stock alert
  lowStockAlert: (items) => ({
    subject: 'تنبيه: انخفاض المخزون - نظام Bastira',
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif;">
        ...
        <hr>
        <small>هذه رسالة تلقائية من نظام Bastira</small>
      </div>
    `
  }),
  ...
  dailyReport: (data) => ({
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif;">
        ...
        <hr>
        <small>تقرير تلقائي من نظام Bastira</small>
      </div>
    `
  }),
  ...
  userCreated: (user, password) => ({
    subject: 'تم إنشاء حسابك في نظام Bastira',
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif;">
        <h2>مرحباً ${user.name}</h2>
        <p>تم إنشاء حسابك في نظام Bastira بنجاح.</p>
        ...
        <hr>
        <small>رسالة من نظام Bastira</small>
      </div>
    `
  })
};
