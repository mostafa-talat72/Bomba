...
const deviceController = {
  ...
  createDevice: async (req, res) => {
    try {
      const { name, number, type, status, controllers } = req.body;
      ...
      const prefix = (type || 'playstation') === 'playstation' ? 'ps' : 'pc';
      const deviceNumber = `${prefix}${number}`;

      const existingDevice = await Device.findOne({ number: deviceNumber });
      if (existingDevice) {
        return res.status(400).json({
          success: false,
