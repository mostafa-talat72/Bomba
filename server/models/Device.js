...
deviceSchema.pre('save', function(next) {
  if (this.isNew || this.isModified('number') || this.isModified('type')) {
    ...
    if (typeof this.number === 'string' && (this.number.startsWith('ps') || this.number.startsWith('pc'))) {
      return next();
    }

    // Extract the numeric part if it's a string
    const numericPart = typeof this.number === 'string' ? this.number.replace(/[^0-9]/g, '') : this.number.toString();
